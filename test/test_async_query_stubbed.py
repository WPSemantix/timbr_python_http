...
# ---------------------------------------------------------------------------
# run_query(async_mode=True) — submit + poll
# ---------------------------------------------------------------------------

@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_sends_x_async_header(mock_post, mock_get):
    mock_post.return_value = _mock_202()
    mock_get.return_value = _mock_get_completed()
    timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, MOCK_TOKEN, MOCK_QUERY, async_mode=True)
    _, kwargs = mock_post.call_args
...
@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_returns_data_on_immediate_complete(mock_post, mock_get):
    mock_post.return_value = _mock_202()
    mock_get.return_value = _mock_get_completed([{'id': 42}])
    result = timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, MOCK_TOKEN, MOCK_QUERY, async_mode=True)
    assert result == [{'id': 42}]


@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_sends_api_key_header(mock_post, mock_get):
    mock_post.return_value = _mock_202()
    mock_get.return_value = _mock_get_completed()
    timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, MOCK_TOKEN, MOCK_QUERY, async_mode=True)
    _, kwargs = mock_post.call_args
...
@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_jwt_headers(mock_post, mock_get):
    mock_post.return_value = _mock_202()
    mock_get.return_value = _mock_get_completed()
    timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, 'jwt_token', MOCK_QUERY,
                    is_jwt=True, jwt_tenant_id='tenant1', async_mode=True)
    _, kwargs = mock_post.call_args
...
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_raises_on_non_202(mock_post):
    mock = MagicMock()
...
@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_with_datasource(mock_post, mock_get):
    mock_post.return_value = _mock_202()
    mock_get.return_value = _mock_get_completed()
    timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, MOCK_TOKEN, MOCK_QUERY, datasource='my_ds', async_mode=True)
...
@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_polls_until_completed(mock_post, mock_get, mock_sleep):
    mock_post.return_value = _mock_202()
    ...
    result = timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, MOCK_TOKEN, MOCK_QUERY,
                             async_mode=True, poll_interval=1.0)
    assert result == [{'id': 99}]
...
@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_raises_on_server_error(mock_post, mock_get):
    mock_post.return_value = _mock_202()
...
@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_raises_timeout(mock_post, mock_get, mock_sleep, mock_monotonic):
    mock_post.return_value = _mock_202()
    ...
    with pytest.raises(TimeoutError, match='did not complete within'):
        timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, MOCK_TOKEN, MOCK_QUERY,
                        async_mode=True, timeout=10.0)


@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_raises_on_404(mock_post, mock_get):
    mock_post.return_value = _mock_202()
...
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_raises_when_no_response_id(mock_post):
    mock = MagicMock()
    mock.status_code = 202
...
@patch('pytimbr_api.timbr_http_connector.requests.get')
@patch('pytimbr_api.timbr_http_connector.requests.post')
def test_async_mode_polls_correct_url(mock_post, mock_get):
    mock_post.return_value = _mock_202()
    mock_get.return_value = _mock_get_completed()
    timbr.run_query(MOCK_URL, MOCK_ONTOLOGY, MOCK_TOKEN, MOCK_QUERY, async_mode=True)
